...
import { trackBookmarkSelect, trackClearDeck, trackCreateBookmark } from '@/lib/analytics';
...
export default function Bookmarks({ 
  ...
  const [bookmarkToDelete, setDeckToDelete] = useState<Bookmark | null>(null);
  const [isEditModalOpen, setIsEditModalOpen] = useState(false);
  const [bookmarkToEdit, setDeckToEdit] = useState<Bookmark | null>(null);
  const [showDeckDropdown, setShowBookmarkDropdown] = useState<string | null>(null);
  ...
  const handleChevronClick = (bookmark: Bookmark, e: React.MouseEvent) => {
    e.stopPropagation();
    setShowBookmarkDropdown(showDeckDropdown === bookmark.id ? null : bookmark.id);
  };
